...
This is an additive, read-only view over the board. Scans pass ``limit=None``
so a board with more than 100 cells of a type is reduced correctly.
"""
...
def _collect_dep_ids(task: Cell) -> list[str]:
    """Dependency ids from both task refs and the legacy data.depends_on list."""
    dep_ids: list[str] = []
    seen: set[str] = set()
...
def _legacy_events_for_task(board: HiveBoard, task_id: str) -> list[Cell]:
    ...
    for ctype in _LEGACY_EVENT_TYPES:
        for cell in board.query(type=ctype, limit=None):
...
def _contract_has_result(board: HiveBoard, contract_id: str) -> bool:
    return any(cell.type == "result" for cell in board.refs(contract_id))


def _result_completes_task(board: HiveBoard, cell: Cell, task_id: str) -> bool:
    """True if a result cell completes this task, directly or via a contract."""
    if cell.data.get("task_id") == task_id:
        return True
...
def _apply_event(
    ...
    *,
    ...
    if cell.type == "verified" and (
        cell.data.get("task_id") == task_id
        or task_id in cell.refs
        or (result_id is not None and result_id in cell.refs)
    ):
...
def get_task_state(
    ...
    *,
    ...
    # If multiple contracts exist but no explicit result event matched yet,
    # a result attached to any of those contracts still completes the task.
    if status == "WORKING":
        for cell in events:
